...
```bash
pip install "agentcore-sdk[agentcore]"
```

### 2. Swap the configuration backend

Replace the default ``Settings`` class with the enterprise version
which reads from your agentcore control plane:

```python
# Before (open-source)
from agentcore.core import Settings
...
The agentcore SDK ships with a full set of enterprise plugins that
auto-register when the package is installed:

```python
from agentcore.plugins.registry import processor_registry
processor_registry.load_entrypoints("agentcore.plugins")
```
